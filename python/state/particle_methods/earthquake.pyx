...
@cython.cdivision(True)
@cython.boundscheck(False)
def bpf_eq(double [:] obs, double [:] params):
    ...
    cdef double norm_factor, foo_double
    cdef int i, j, k, idx

    # Define parameters
    cdef double mu, phi, sigmav
    mu = params[0]
    phi = params[1]
    sigmav = params[2]
    ...
    for j in range(NPART):
        particles[0 + j * NOBS] = mu + stDev * random_gaussian()
    ...
    for i in range(1, NOBS):
        ...
        # Propagate particles
        for j in range(NPART):
            mean = mu + phi * (particles[i + ancestors[j] * NOBS] - mu)
            particles[i + j * NOBS] = mean + sigmav * random_gaussian()

        # Weight particles
        for j in range(NPART):
            unnorm_weights[j] = poisson_logpdf(obs[i], exp(particles[i + j * NOBS]))
...
@cython.cdivision(True)
@cython.boundscheck(False)
def bpf_eq_corr(double [:] obs, double [:] params, double [:] rvr, double [:] rvp):
    ...
    for i in range(1, NOBS):
        ...
        for j in range(NPART):
            mean = mu + phi * (particles[i + ancestors[j] * NOBS] - mu)
            particles[i + j * NOBS] = mean + sigmav * rvp[i + j * NOBS]
...
@cython.cdivision(True)
@cython.boundscheck(False)
def flps_eq(double [:] obs, double [:] params):
...
@cython.cdivision(True)
@cython.boundscheck(False)
def flps_eq_corr(double [:] obs, double [:] params, double [:] rvr, double [:] rvp):
...
@cython.cdivision(True)
@cython.boundscheck(False)
cdef double poisson_logpdf(double x, double m):
    """Helper for computing the log of the Poisson pdf."""
    cdef double part1 = x * log(m)
    cdef double part2 = -m
    cdef double part3 = -log(factorial(x))
    return part1 + part2 + part3

@cython.cdivision(True)
@cython.boundscheck(False)
cdef double factorial(double n):
    if (n == 1):
        return 1
    else:
