...
helper_plotting <- function(data, result, settings, algorithm, noItersToPlot,
  ...
  obs <- as.numeric(data$observations)
  noIters <- as.numeric(settings$no_iters) - as.numeric(settings$no_burnin_iters)
  ...
  if (length(paramsEstMean) == 2) {
    layout(matrix(c(1, 1, 1, 2, 2, 2, 3, 4, 5, 6, 7, 8), 4, 3, byrow = TRUE))
  }
  ...
  for (k in 1:length(paramsEstMean)) {
    idx <- seq(1, memory_length*noItersToPlot, memory_length)
